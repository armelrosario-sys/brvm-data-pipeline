...
RE_CAPITAUX = re.compile(r"capitaux\s+propres\s*\*?\s*[:|]?\s*([^\n]*)", re.I)
...
def main():
    ...
    for i, (t, l) in enumerate(sorted(recents.items()), 1):
        ...
        try:
            ...
            r.raise_for_status()
            res = extrait(texte_pdf(r.content))
            if not res:
                raise ValueError("ligne « Capitaux propres » absente du rapport")
    ...
    print(f"\n{SORTIE} écrit — {len(trouves)} extraction(s), {len(garde)} saisie(s) manuelle(s)")
    if echecs:
        print("Sans capitaux propres, à saisir à la main si besoin :\n  " + "\n  ".join(echecs))
